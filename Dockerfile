...
# Apply available Debian security patches. The pinned base digest lags Debian
# point releases, so the release Trivy gate (CRITICAL,HIGH, fixed-only) blocks
# on base-OS CVEs that upstream has already fixed; upgrading here clears them.
